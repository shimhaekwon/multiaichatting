...
7. `227-설계변경-floor-random직렬화-및-순서표시.md` — **(설계 변경안 · v1.1)** race(속도 경쟁 floor) → **랜덤 순서 직렬** + Roster 순번 배지 + 자동 대화 **바퀴 로테이션·라운드 종료선**(D-D). **구현·라이브검증·정본([221]§R2·[222]) 정합 완료**.
8. `228-진입로비-런타임모델선택.md` — **(설계 변경안 · v1.0)** 진입 **로비**에서 로컬 Ollama 모델 목록(`/api/tags`) 선택 + 인원 가감·이름·영속화 + **연결 보강**(주소 지정·타임아웃·진단·재연결). **구현·라이브검증 완료**.

## 문서 목록

| 문서 | 영역 | 핵심 | 버전 | 수정일 | 상태 |
|------|------|------|------|--------|------|
| `221-제품개요-및-핵심규칙.md` | 제품 개요·규칙 | 4규칙(R1~R4) 정밀 정의·범위·모호함 해소(D1~D5) | v1.0 | 2026-06-02 | 정본(규칙 §3 최우선) · ¹ |
| `222-coordinator-상태머신.md` | Coordinator | 단일 floor 루프·race-free·abort 가드·whisper·driver seam | v1.0 | 2026-06-02 | 정본 · ¹ |
| `223-데이터모델-로그포맷.md` | 데이터 모델 | 타입·저장 스키마·MD 포맷·whisper 미저장 | v1.0 | 2026-06-02 | 정본 |
| `224-아키텍처-스택-단계.md` | 아키텍처·스택·단계 | 3레이어·Vite+React+TS·P0/P1/P2 완료기준·carry-forward·멀티유저 경로 | v1.0 | 2026-06-02 | 정본 |
| `225-UI-UX-설계.html` | UI/UX | 90s 디자인 토큰·레이아웃·컴포넌트 매핑·상태 시각화·귓속말 UI·인터랙티브 목업 | v1.0 | 2026-06-02 | 정본(O1 확정) |
| `226-구현계획-P0.md` | 구현 계획 | 착수게이트·스캐폴드·빌드순서·Vitest·DoD·마일스톤 | v1.0 | 2026-06-02 | 정본 |
| `227-설계변경-floor-random직렬화-및-순서표시.md` | 설계 변경 | race→**랜덤 직렬 floor**+순번 배지 · 바퀴 로테이션·라운드 종료선(D-D) | v1.1 | 2026-06-05 | 구현·정합 완료 |
| `228-진입로비-런타임모델선택.md` | 설계 변경 | 진입 로비(런타임 모델 선택)·인원·영속 · Ollama 연결 보강(주소·타임아웃·진단·재연결) | v1.0 | 2026-06-05 | 구현·라이브검증 완료 |

> ¹ 227/228 반영해 **개정 완료**(2026-06-05): [221]§R2/§4(랜덤 직렬·바퀴 로테이션·라운드), [222] v4(floor 흐름·`speakOne`·`pickAutoSpeaker`·`dispose`).
>
> **전체 변경 이력**: [`CHANGELOG.md`](./CHANGELOG.md) · 각 문서 상세 = 문서 말미 "변경 이력".
