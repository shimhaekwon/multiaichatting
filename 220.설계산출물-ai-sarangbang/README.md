...
7. `227-설계변경-floor-random직렬화-및-순서표시.md` — **(설계 변경안 · v1.0)** race(속도 경쟁 floor) → **랜덤 순서 직렬** + Roster 순번 배지. 라이브 디버깅("응답 없음") 대응. **구현·라이브검증 완료**. [221]§R2·[222] 개정 예정.
8. `228-진입로비-런타임모델선택.md` — **(설계 변경안 · v1.0)** 진입 **로비**에서 로컬 Ollama 모델 목록(`/api/tags`) 선택 + 인원 가감·이름·영속화. 검토 반영 완료·**구현 대기**.
...
| `227-설계변경-floor-random직렬화-및-순서표시.md` | 설계 변경 | race→**랜덤 직렬 floor**+순번 배지 · 라이브 디버깅 대응 · 검토 반영 | v1.0 | 2026-06-04 | 구현 대기 |

> ¹ 227 구현 시 개정 예정: [221]§R2(속도 경쟁→랜덤 직렬), [222](floor 흐름·`speakOne`·`onOrder`).
>
